...
def battle(enemy):
    ...
    if enemy[0] <= 0:
        health_increase = r.randint(2, max(4, enemy[1]))
    ...
    if len(room):
        ...
        for i in range(length):
            ...
            if entity[0] == "enemy":
                ...
                print("There's the \033[93mrelic\033[0m in the room!")
                print("You quickly pocket it.")
                print("Now you just need to \033[31mget out\033[0m...")
                ...
                player["damage"] += 1
        if depth == 1:
            print("\033[33mThe entrance to the dungeon looms behind you.\033[0m")
        elif depth == 2:
            print("\033[33mThe entrance to the dungeon is still visible, but it's getting further.\033[0m")
        elif depth == 3:
            print("\033[33mYou don't see the entrance anymore, but you are sure you can go back the way you came.\033[0m")
        elif depth == 4:
            print("\033[33mYou are deep in the dungeon, but you can still go back the way you came.\033[0m")
        elif depth == 5 and doors_out > 0:
            print("\033[33mYou are very deep in the dungeon. Does it ever end?\033[0m")
        elif depth == 5 and doors_out == 0:
            print("\033[33mYou are very deep in the dungeon. However, it seems like there are no more ways to go deeper...\033[0m")
        else:
            print("\033[33mYou have a feeling this is the end of the dungeon... Nothing can be deeper than this.\033[0m")
        if doors_out == 1:
        ...
        input("The room is \033[37mempty\033[0m and it's a \033[90mdead end\033[0m. Perhaps you should go back...")
        return 0
...
def generate_dungeon():
    ...
    address = dungeon[0]
    r.seed(int(time()))
    maxdepth = r.randint(4, 6)
    moveorder = [1] #this list is a sequence of depths of the rooms
    iteration = 0
    treasure_added = 0
    while iteration < len(moveorder):
        depth = moveorder[iteration]
        item_count = 1 + r.randint(1, 10)//5 #how many items are there in the room, including ways to other rooms
        ...
        for j in items:
            ...
            if depth < 3:
                if pull <= 99-50*j:
                    ...
                    if 0 == treasure_added and depth >= 2:
...
depth = 1
moves = [None]*6
print("\033[33mThe dungeon door opens up, revealing an empty room with many doors leading deeper.")
print("The rumours say there's an ancient \033[93mrelic\033[33m inside, granting many powers to the one who wields it.")
    ...
    if player_action == 0:
        ...
        if depth == 1:
            if player["has_treasure"]:
                ...
                print("You can't just walk away without the relic...")
                depth = 1
        else:
            depth -= 1
